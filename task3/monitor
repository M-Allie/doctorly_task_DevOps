If you have multiple Ubuntu prod instances, How would you monitor them? What would be your monitoring strategy?
        ...
        The absolute first issue is understanding what you want to monitor, why the information is important and most importantly what should be done wityh the info. 
            
            Is only OS metrics needed?
            Is application metrics needed?
            Do logs need to me monitored or exported?
        
        Next a mechism to alert those who need to know and/or take action based on criteria. Ideally your monmitoring will be linked to automation that will resolve know issues.
        ...
        If the monitoring targets do vary, a third party solution like https://www.zabbix.com/ whould be my choise. This will allow for a single interface for minitoring. For log export and anylisis, an ELK stack could be used: https://www.elastic.co/what-is/elk-stack

        Additionally external or service level monitor is needed. An example of this would a system that check http response codes and service responce times.
